...
class PlayBar extends React.Component {
    ...
    componentDidUpdate(prevProps,prevState){
        ...
        localStorageFn.setItem('currentMusic',JSON.stringify(curMusic));
    }
    //点击修改播放状态
    changePlayState(){
    ...
    deleteSelectedSong(index){
        ...
        // let curMusicIndex = this.props.currentMusic.index;
        // if(playState){
        ...
        let copy = playlist.concat();
        copy.splice(index,1);
        this.props.actions.removeSong(index);
        localStorageFn.setItem('playlist',JSON.stringify(copy));
    }
    //清空歌单列表(传递给子组件)
    clearList(){
        this.props.actions.changePlaylist([]);
        localStorageFn.setItem('playlist',JSON.stringify([]));
    }
    //点击修改播放模式 单曲 循环 还是随机
    changePlayMode(){
        let mode = this.props.playMode;
...
class PlayPanel extends React.Component {
    ...
    componentDidMount(){
        ...
        let nowIndex = this.props.currentMusic.index; //当前歌曲的索引
        let len = this.props.playlist.length;
        let scale;
        if(len){
            if(nowIndex>=5 && nowIndex < len-5){
                scale = (nowIndex) * 28 /(scrollCon.offsetHeight -  conWrap.offsetHeight);
                scrollCon.style.top = -(nowIndex-4) * 28 + 'px';
                scrollbar.style.top = scale *(barWrap.offsetHeight -  scrollbar.offsetHeight)+'px'
            }else if(nowIndex >= len-5){
                console.log(barWrap.offsetHeight -  scrollbar.offsetHeight);
                scrollCon.style.top = conWrap.offsetHeight - scrollCon.offsetHeight+ 'px';
                scrollbar.style.top = (barWrap.offsetHeight -  scrollbar.offsetHeight)+'px'
            }
        }
        console.log(this.props.currentMusic.index);
        this.scrollMove();
    }
                                ...
                                <ul ref={scrollCon =>this.scrollCon=scrollCon} className="slist">
                                    {
                                        this.props.playlist.map((item,index)=>{
                                            //console.log(item)
                                            return (
                                                <li key={item.id} className={this.props.currentMusic.index ==index?'active':''} onClick={ev=>{this.playSong(index)}}>
                                                    <div className="col1 col">
                                                        <div className="play" style={{display:this.props.currentMusic.index ==index?'block':'none'}}></div>
                                                    </div>
                                                    <div className="col2 col">{item.name}</div>
                                                    ...
                                                    <div className="col4 col"><a href="#">{item.ar?item.ar[0].name: item.artists[0].name}</a></div>
